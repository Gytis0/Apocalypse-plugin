...
public class Pathing {
    ...
    public static Block findLocationForPathBuildingUp(Location targetLoc, LivingEntity builder) {
        ...
        for (Block b : possibleResults) {
            if (b.getLocation().distance(builder.getLocation()) < smallestDistance && Pathing.isLocationReachable(builder, b.getLocation()) && Pathing.isBlockClear(b)) {
    ...
    public static boolean isBlockReachable(LivingEntity origin, Block block, double range) {
        Mob mob = (Mob) origin;
        Pathfinder.PathResult path = mob.getPathfinder().findPath(block.getLocation());

        Location closestLocation;
        if (path == null) return false;
        closestLocation = path.getFinalPoint();

        return closestLocation != null && closestLocation.add(0, 1, 0).distance(block.getLocation()) <= range;
    }
}